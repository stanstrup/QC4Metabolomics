...
dir <- "/data/SYSDIET/SysDiet Kati Mouse and Pig tissues biofluids.pro/SysDiet Katti_Plate II_Mouse urine and pig biofluid samples_250910.PRO"

to_fix <- list.files(dir,"\\.mzML$" ,full.names = TRUE, recursive = TRUE)

project <- "SYSDIET-kati-mouse-urine-biofluid"
instrument <- "Qnew"
date_format <- "%d%m%Y"
...
sample_list <- list.files(dir,"\\.SPL", full.names = TRUE, recursive = TRUE) %>% 
  ...
  map(~..1[,c("VERSION", "FILE.NAME", "FILE.TEXT", "MS.FILE", "INLET.FILE","SAMPLE.LOCATION", "TYPE", "INJ.VOL", "Index")]) %>% 
  do.call(rbind,.) %>% 
  as_tibble
