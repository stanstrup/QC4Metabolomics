...
Internally (inside the docker network) the database runs on port 3306.
On the host it is accessible at 12345 (unless you changed it in the
docker compose file). If you are working on the host directly you can
access on this port.\
...
Start by making a copy of `settings_demo.env`. Call it for example
`settings_remote.env`. Now you want to edit the settings to connect to
the server. The relevant parameters are:
