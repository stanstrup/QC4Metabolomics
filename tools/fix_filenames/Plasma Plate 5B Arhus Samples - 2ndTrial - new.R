dir <- c("/data/DanORC studies/PPL5B Aarhus_plasma samples/Plasma Plate 5B Arhus Samples_neg_2ndTrial_new.PRO", "/data/DanORC studies/PPL5B Aarhus_plasma samples/Plasma Plate 5B Arhus Samples_pos_2ndTrial_new.PRO")
...
project <- "Plasma Plate 5B Arhus Samples - 2ndTrial - new"
#instrument <- "Qnew"
date_format <- "%d%m%y"
...
if(nrow(file_tab)!=0) writeLines(unique(mz_files[!(mz_files %in% file_tab$to_fix)]), out_file)
