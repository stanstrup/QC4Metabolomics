dir <- c("/data/Pig studies/Pig studies - Pingping/AB metabolomics.PRO")
...
SPL <- list.files(dir,"\\.SPL$", full.names = TRUE, recursive = TRUE, ignore.case = TRUE)
SPL_new <- gsub("&","_", SPL)
...
file_tab <- tibble(to_fix, desc = desc, inlet = sample_list$INLET.FILE[idx]) %>% 
              mutate(date = gsub(".*/(.*)_.*\\.mzML$", "\\1" ,to_fix)) %>%
  
              mutate(date_chars = nchar(date)) %>% 
              mutate(
                    date = case_when(
                      date_chars==10 ~ as_date(date, format = "%Y-%m-%d"),
                      date_chars==8 ~ as_date(date, format = "%d%m%Y"),
             ...
             # mutate(date = format(as_date(date, format = date_format), "%Y%m%d")) %>%
              mutate(batch_seq_nr = gsub(".*/.*_(.*)\\.mzML$", "\\1" ,to_fix)) %>%
              # slice(1:2) %>% 
