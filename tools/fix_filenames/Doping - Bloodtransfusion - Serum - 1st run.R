dir <- c("/data/Doping/Bloodtransfusion/Serum/1st run")
...
project <- "Doping - Bloodtransfusion - Serum - 1st run"
#instrument <- "Qnew"
#date_format <- "%Y-%m-%d"
...
# # Seems to be standards missing from the sample list
...
file_tab <- tibble(to_fix, desc = desc, inlet = sample_list$INLET.FILE[idx]) %>% 
              ...
              mutate(
                    date = case_when(
                      ...
                      date_chars==9 & grepl("Q$", date) ~ as_date(date, format = "%d%m%YQ"),
                      .default = NA
                    )
                  ) %>%  
