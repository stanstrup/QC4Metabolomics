Admin <- function(input, output, session, global_instruments_input) {
    ...
    # Modules that use the file_schedule priority queue -----------------------
    ...
    observeEvent(input$reset_priority_btn, {
        ...
        selected <- input$modules_to_reset
        ...
        results <- map_chr(selected, function(mod) {
            tryCatch({
                ...
                n <- dbExecute(
                    con,
                    paste0("UPDATE file_schedule SET priority = 1 WHERE module = '",
                           mod, "' AND priority = -1")
                )
    ...
    observeEvent(input$remove_missing_btn, {
        ...
        result_msg <- tryCatch({
            ...
            file_tbl <- mutate(file_tbl, file_found = file_exists(path))
        ...
        tryCatch({
            list(
                ...
                schedule  = dbGetQuery(pool, "
                    SELECT module,
                        SUM(CASE WHEN priority =  1 THEN 1 ELSE 0 END) AS pending,
                        SUM(CASE WHEN priority = -1 THEN 1 ELSE 0 END) AS done,
                        COUNT(*) AS total
                    FROM file_schedule
                    GROUP BY module
                    ORDER BY module"),
    ...
    # Preview: show files whose <field> matches the "from" value --------------
    ...
    # Apply: UPDATE <field> WHERE <field> = from_val --------------------------
