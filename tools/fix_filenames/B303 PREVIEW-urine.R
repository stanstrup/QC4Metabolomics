dir <- "/data/B303 PREVIEW/B303 Urine"

to_fix <- list.files(dir,"\\.mzML$" ,full.names = TRUE, recursive = TRUE)

to_fix <- grep("CALIBRATION|TEST|18052016-", to_fix, value = TRUE, invert = TRUE)

project <- "B303 PREVIEW-urine"
instrument <- "Qquat"
date_format <- "%d%m%YQ"
...
file_tab <- tibble(to_fix, desc = desc) %>% 
              ...
              mutate(date = gsub("220552018", "22052018", date)) %>% # fix bad date
