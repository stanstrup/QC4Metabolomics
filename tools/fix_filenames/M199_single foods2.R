dir <- "/data/OPUS studies/M199 - Nordic meals study/M199_Single foods_2"

to_fix <- list.files(dir,"\\.mzML$" ,full.names = TRUE, recursive = TRUE)

to_fix <- grep("CALIBRATION|TEST", to_fix, value = TRUE, invert = TRUE)

project <- "OPUS-M199-Single foods2"
date_format <- "%Y%m%d"
...
file_tab <- tibble(to_fix, desc = desc, inlet = sample_list$INLET.FILE) %>% 
