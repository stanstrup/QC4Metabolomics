dir <- c("/data/PRIMA projects/PRIMA.pro/PRIMA urin metabolomics/neg_mode")
...
desc <- gsub(".*/.*?_.*?_.*?_.*?_.*?_.*?_(.*)\\.mzML$","\\1",to_fix)
...
file_tab <- tibble(to_fix, desc = desc, inlet = "Metabolomics_version2_7min_05flow_5uL") %>% 
              mutate(date = gsub(".*/mzML/.*?_(.*?)_.*\\.mzML$", "\\1" ,to_fix)) %>%
              ...
              mutate(batch_seq_nr = gsub(".*/.*?_.*?_.*?_.*?_(.*?)_.*\\.mzML$", "\\1" ,to_fix)) %>%
              # slice(1:2) %>% 
