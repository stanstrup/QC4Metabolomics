...
- `QC4METABOLOMICS_msconvert_outdir_prefix`: The output folder relative to the source files. the default, `/../mzML`, steps one folder back, makes a new mzML folder and puts the converted files there.
  ...
  **Demo setup note:** If you are using the demo setup and placing files directly in `demo/data/`, this default will not work — `demo/data/` is the root of the container mount and there is nothing to step back to. Instead, place your raw files in a subdirectory, e.g. `demo/data/raw/`, and list them in `raw_filelist.txt` with the subdirectory prefix:
  ```
