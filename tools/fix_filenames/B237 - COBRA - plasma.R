dir <- c("/data/DanORC studies/B237_COBRA plasma samples.pro")
...
file_tab <- tibble(to_fix, desc = desc, inlet = sample_list$INLET.FILE[idx]) %>% 
              ...
              mutate(
                    instrument = case_when(
                      ...
                      inlet == "washing_step"                         ~ "Qold",
                      inlet == "mkri6min04flow_10uL" ~ "Qold",
                      inlet == "mkri7-5min04flow_7uL" ~ "mkri7-5min04flow-7uL",
