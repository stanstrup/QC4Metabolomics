...
We go into the `EIC` lists, merge the lists while we add a row to show which m/z slice each belongs to.
While we are at it we change the new `mz` column to a factor.
```{r 'multiple  EIC 5'}
...
So now we can get rid of those nested tables.
First we remove the `ranges` and the `raw` columns because we cannot unnest everything together.
```{r 'multiple  EIC 6'}
